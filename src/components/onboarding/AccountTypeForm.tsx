...
interface AccountTypeFormProps {
  onNext: (data: { accountType: { userType: 'auditor' | 'project-owner' } }) => void;
  data: {
...
export function AccountTypeForm({ onNext, data }: AccountTypeFormProps) {
  const [selectedType, setSelectedType] = React.useState<'auditor' | 'project-owner'>(
    data.userType || 'auditor'
  );

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onNext({ accountType: { userType: selectedType } });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">
          Select the role that best describes how you'll be using the Hawkly platform
        </p>
      </div>

      <RadioGroup
        value={selectedType}
        onValueChange={(value: 'auditor' | 'project-owner') => setSelectedType(value)}
      ...
      >
        <div className="relative">
          ...
          <Label
            htmlFor="auditor"
            className={cn(
              "flex flex-col h-full items-center justify-between rounded-md border-2 border-muted bg-white p-4 hover:bg-gray-50 cursor-pointer",
              "peer-data-[state=checked]:border-hawkly-primary",
              selectedType === "auditor" ? "border-hawkly-primary" : "border-gray-200"
            )}
          >
            <div className="mb-4">
              <svg 
            ...
            <CardTitle className="text-xl mb-2">Auditor</CardTitle>
            <CardDescription className="text-center text-sm text-gray-500">
              Security researchers, auditors and bug bounty hunters helping secure web3 projects
            </CardDescription>
          </Label>
        </div>
        ...
        <div className="relative">
          ...
          <Label
            ...
            className={cn(
              "flex flex-col h-full items-center justify-between rounded-md border-2 border-muted bg-white p-4 hover:bg-gray-50 cursor-pointer",
              "peer-data-[state=checked]:border-hawkly-primary",
              selectedType === "project-owner" ? "border-hawkly-primary" : "border-gray-200"
            )}
          >
            <div className="mb-4">
              <svg 
            ...
            <CardTitle className="text-xl mb-2">Project Owner</CardTitle>
            <CardDescription className="text-center text-sm text-gray-500">
              Developers and teams building web3 projects that need security expertise
            </CardDescription>
          </Label>
        </div>
      </RadioGroup>
